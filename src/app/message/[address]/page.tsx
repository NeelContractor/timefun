...
// import { useEffect, useMemo, useRef, useState } from "react";
// import { Send, Clock, Circle } from "lucide-react";
...
// export default function Page() {
//     const { conversationAccounts, creatorProfileAccounts, sendMessageHandler, messageAccounts } = useTimeFunProgram();
//     const { publicKey } = useWallet();
//     const [profile, setProfile] = useState<ProfileType>();
//     const [message, setMessage] = useState("");
//     const [conversationPubkey, setConversationPubkey] = useState<PublicKey>();
...
//     useEffect(() => {
//         if (conversationAccounts.data && publicKey && profile) {
//             const conversation = conversationAccounts.data.find(
//                 (convo) => 
//                     convo.account.creator.equals(profile.creator) && 
...
//             }
//         }
//     }, [conversationAccounts.data, publicKey, profile]);
...
//                                 {profile?.basePerToken ? `${(profile.basePerToken.toNumber() / 1e9).toFixed(2)} SOL/${profile.charsPerToken.toNumber()} Char` : "Loading..."}
//                             </span>
//                         </div>
//                     </div>
//                 </div>
...
//                                                             : 'bg-gradient-to-br from-pink-950/40 to-purple-950/40 border border-pink-500/20 text-white'
//                                                     }`}
//                                                 >
...
//                                                 {/* Timestamp & Token Info */}
//                                                 <div className={`flex items-center gap-2 mt-1 px-2 text-xs text-gray-500 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
//                                                     <span>{formatTimestamp(msg.account.timestamp)}</span>
//                                                     {msg.account.tokensBurned.toNumber() > 0 && (
//                                                         <>
//                                                             <span>•</span>
